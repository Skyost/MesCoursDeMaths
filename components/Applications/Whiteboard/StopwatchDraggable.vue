  ...
  >
    <ski-columns class="h-100 ms-0 me-0">
      <ski-column class="duration-column" width="2">
        <ski-button variant="success" @click.native="changeSeconds(seconds + 600)">
          <ski-icon icon="plus-lg" />
        </ski-button>
        <span class="duration" v-text="tenMinuteCount" />
        <ski-button variant="danger" @click.native="changeSeconds(seconds - 600)">
          <ski-icon icon="dash-lg" />
        </ski-button>
      </ski-column>
      <ski-column class="duration-column" width="2">
        <ski-button variant="success" @click.native="changeSeconds(seconds + 60)">
          <ski-icon icon="plus-lg" />
        </ski-button>
        <span class="duration" v-text="minuteCount" />
        <ski-button variant="danger" @click.native="changeSeconds(seconds - 60)">
          <ski-icon icon="dash-lg" />
        </ski-button>
      </ski-column>
      ...
      <ski-column class="duration-column" width="2">
        <ski-button variant="success" @click.native="changeSeconds(seconds + 10)">
          <ski-icon icon="plus-lg" />
        </ski-button>
        <span class="duration" v-text="tenSecondCount" />
        <ski-button variant="danger" @click.native="changeSeconds(seconds - 10)">
          <ski-icon icon="dash-lg" />
        </ski-button>
      </ski-column>
      <ski-column class="duration-column" width="2">
        <ski-button variant="success" @click.native="changeSeconds(seconds + 1)">
          <ski-icon icon="plus-lg" />
        </ski-button>
        <span class="duration" v-text="secondCount" />
        <ski-button variant="danger" @click.native="changeSeconds(seconds - 1)">
          <ski-icon icon="dash-lg" />
        </ski-button>
      </ski-column>
      <ski-column class="ms-auto p-0 d-flex justify-content-center" width="1">
        <ski-button :disabled="seconds <= 0" @click.native="togglePlay">
...
<script>
import { SkiButton, SkiIcon, SkiColumns, SkiColumn } from 'skimple-components'
