...
import { ContentGeneratorSettings, contentGeneratorSettings, MarkdownResource } from '../site/content-generator'
import { normalizeString, getFileName, getDirectories, generateChecksum } from '../utils/utils'
import { ModuleOptions as CommitShaFileGeneratorModule } from './generate-commit-sha-file'
...
export default defineNuxtModule<ModuleOptions>({
  ...
  setup: async (options, nuxt) => {
    ...
    const tempDir = await downloadRemoteDirectory(
      ...
      directories,
      lessonsDirectory,
      latestCommitShaFilename ? resolver.resolve(srcDir, 'content', latestCommitShaFilename) : null
    )
    if (tempDir) {
...
async function downloadPreviousBuild (resolver: Resolver, srcDir: string, github: GithubData, contentGenerator: ContentGeneratorSettings) {
  try {
...
async function downloadRemoteDirectory (resolver: Resolver, github: GithubData, directories: SiteDirectories, lessonsDirectory: string, latestCommitShaFile: string | null) {
  ...
  const repoDirectory = getDirectories(tempDirectory)[0]
...
function toString (slug: string, root: HTMLElement, linkedResources: MarkdownResource[]) {
